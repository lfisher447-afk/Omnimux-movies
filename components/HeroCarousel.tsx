...
type MovieDetails = { id: number; backdrop_path: string; poster_path: string; title?: string; name?: string; overview: string; media_type?: string, credits?: { cast: any[] } };
...
export function HeroCarousel({ movies }: { movies: MovieDetails[] }) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => setIndex(p => (p + 1) % 5), 8000);
    return () => clearInterval(interval);
  }, [movies, detailedView]);

  const onDragEnd = (event: any, info: any) => {
    if (info.offset.x < -100) setIndex(p => (p + 1) % 5);
    else if (info.offset.x > 100) setIndex(p => (p - 1 + 5) % 5);
  };
  ...
  if (!movies.length) return <div className="h-[85vh] bg-black shimmer-bg" />;
          ...
          <div className="absolute inset-0 max-w-[1800px] mx-auto px-6 lg:px-16 flex items-center">
             <div className="max-w-2xl z-10">
               <motion.div initial="hidden" animate="visible" variants={{ hidden: {}, visible: { transition: { staggerChildren: 0.15 } } }}>
                 <motion.div variants={{hidden:{opacity:0,y:20},visible:{opacity:1,y:0}}} className="flex gap-3 mb-6"><span className="bg-red-600 text-white px-3 py-1 rounded text-xs font-black uppercase"><Flame/> Trending</span></motion.div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-black/80 backdrop-blur-2xl z-50 flex items-center justify-center p-8">
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="w-full max-w-4xl h-[70vh] bg-black border border-white/10 rounded-3xl flex overflow-hidden">
              ...
              <div className="flex-1 p-8 overflow-y-auto custom-scrollbar">
                ...
                <div className="flex items-center gap-4 my-4"><span className="flex items-center gap-1 text-yellow-400"><Star/> {detailedView.vote_average.toFixed(1)}</span> <span className="text-gray-400">{detailedView.release_date}</span></div>
                <p className="text-gray-300 mb-6">{detailedView.overview}</p>
                <h3 className="font-bold text-xl mb-3">Cast</h3>
                <div className="flex gap-4 overflow-x-auto pb-4">
                  {detailedView.credits?.cast.slice(0, 10).map(c => <div key={c.id} className="w-24 text-center"><img src={`https://image.tmdb.org/t/p/w200${c.profile_path}`} className="w-24 h-24 rounded-full object-cover mb-2"/><p className="text-xs line-clamp-1">{c.name}</p></div>)}
                </div>
